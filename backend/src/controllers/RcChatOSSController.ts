...
export async function show(_req: Request, res: Response): Promise<Response> {
  if (!process.env.RC_CHAT_REGISTRY_URL) {
    return res.status(200).json({ disabled: "true" });
  }

  const registry = JSON.parse(
    await GetCompanySetting(1, "rc_chat_registry", "{}")
  );
...
export async function store(req: Request, res: Response): Promise<Response> {
  ...
  const registryData = {
    ...
    email: process.env.EMAIL_ADDRESS || "admin@rc-chat.host",
    version: GitInfo.branchName || GitInfo.commitHash
  };

  await axios.post(process.env.RC_CHAT_REGISTRY_URL, registryData);
