...
const createConnectionsRouter = require('./connections');
...
function createApiRouter(pool, io, JWT_SECRET) {
  ...
  // --- Rotas Autenticadas para Atendentes ---
  ...
  router.post('/chats/:chatId/transfer', authenticateToken, async (req, res) => {
    const { companyId, name: currentUserName } = req.user;
  ...
  adminRouter.get('/companies/:companyId/users', async (req, res) => {
    ...
    try {
      const result = await pool.query('SELECT id, name, email, role FROM users WHERE company_id = $1', [companyId]);
      res.json(result.rows);
    } catch (error) {
      res.status(500).json({ message: 'Erro interno do servidor.' });
    }
