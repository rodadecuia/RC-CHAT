...
const RcChatRegistry = ({onRegister}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/rc-chat/registry', registryData);
      ...
      <Grid container spacing={3}>
        <Grid item xs={12}>
            <Typography component="h2" variant="h6" gutterBottom>
              {i18n.t("rcchat.registration.header")}
            </Typography>
            <p>{i18n.t("rcchat.registration.description")}</p>
        </Grid>
        <Grid item xs={12} md={4}>
          <TextField
            className={classes.fullWidth}
            label={i18n.t('rcchat.registration.name')}
            name="name"
            value={formData.name}
        ...
        <Grid item xs={12} md={3}>
          <SelectCountry
            className={classes.fullWidth}
            label={i18n.t('rcchat.registration.country')}
        ...
        <Grid item xs={12} md={3}>
          <TextField
            className={classes.fullWidth}
            label={i18n.t('rcchat.registration.phoneNumber')}
            name="phoneNumber"
        ...
        <Grid item xs={12} md={2}>
          <Button className={classes.fullWidth} type="submit" variant="contained" color="primary">
            {i18n.t('rcchat.registration.submit')}
          </Button>
        </Grid>
      </Grid>
    </form>
